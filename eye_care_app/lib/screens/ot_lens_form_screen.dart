...
/// Round 3 Phase 4 — Lens Record. `lens_inventory`/`lens_powers`/
/// `lens_types` all come bundled in `GET .../lens` itself — no need for the
/// separate `/ot/lens-inventory/search` typeahead endpoint here.
      ...
      Expanded(
        child: ListView(
          ...
          children: [
            ...
            TextFormField(controller: _lensNameCtrl, decoration: _deco('Lens Name *')),
            ...
            Row(children: [
              ...
              Expanded(
                child: InkWell(
                  onTap: _pickExpiry,
                  child: InputDecorator(decoration: _deco('Expiry Date'), child: Text(_expiryDate == null ? 'Select' : '${_expiryDate!.year}-${_expiryDate!.month.toString().padLeft(2, '0')}-${_expiryDate!.day.toString().padLeft(2, '0')}')),
                ),
              ),
            ]),
            const SizedBox(height: 10),
            ...
            SwitchListTile(
              value: _implanted,
              onChanged: (v) => setState(() => _implanted = v),
              title: const Text('Implanted', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
              subtitle: const Text('Auto-decrements inventory stock on save', style: TextStyle(fontSize: 11, color: AppColors.textSecondary)),

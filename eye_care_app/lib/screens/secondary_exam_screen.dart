...
class _PrescRow {
  ...
  Map<String, dynamic> toJson() => {
        ...
        if (medicineName.isNotEmpty) 'name': medicineName,
        if (dosageId != null) 'dosage_id': dosageId,
...
class _SecondaryExamScreenState extends State<SecondaryExamScreen> {
  ...
  // Section 2: Vision — flat web keys
  ...
  static const _stepNames = ['C/O', 'K/C/O & H/O', 'Vision', 'PG', 'ST', 'NCT', 'O/E', 'Fundus', 'Dilate', 'Diagnosis', 'Medicine', 'Advice'];
  ...
  Future<void> _loadAll() async {
    ...
    try {
      // Step 1: Masters first (22 parallel calls) — kept separate to avoid
      // competing with exam data fetch on slow connections.
      final formData = await ExamService.instance.loadFormData();

      // Step 2: Secondary exam data fetch.
      final secondary = await ExamService.instance.loadSecondaryExam(widget.patient.id);

      // Step 3: Primary fetch is conditional — only if no secondary exists.
      final primary = secondary == null
          ? await ExamService.instance.loadPrimaryExam(widget.patient.id)
          : null;
      ...
      // Stale disk-cache guard: if diagnoses came back empty but other masters
      // loaded fine, the cache is corrupt/old — force a fresh network pull.
      if (formData.masters.diagnoses.isEmpty && formData.masters.vn.isNotEmpty) {
        _refreshMasters();
      }
    } catch (e) {
  ...
  void _prefill(Map<String, dynamic> exam) {
    ...
    // Primary exam format: medicines in top-level prescriptions relation
    for (final m in (exam['prescriptions'] as List? ?? [])) {
      final row = m as Map<String, dynamic>;
      _medicines.add(_PrescRow(
        medicineId: row['medicine_id'] as int?,
        medicineName: (row['medicine'] as Map<String, dynamic>?)?['name'] as String? ?? '',
        dosageId: row['dosage_id'] as int?,
        routeId: row['route_id'] as int?,
        duration: row['duration'] as String? ?? '',
        quantity: (row['quantity'] as num?)?.toInt() ?? 1,
      ));
    }
    // Secondary exam format: medicines embedded in exam_data.rx
  ...
  Future<void> _save() async {
    ...
    try {
      await ExamService.instance.saveSecondaryExam(widget.patient.id, _buildStepPayload(_currentStep));
      if (!mounted) return;
      _anySaved = true;
  ...
  Map<String, dynamic> _buildStepPayload(int step) {
    ...
    switch (step) {
      ...
      case 6: // O/E
        ...
        if (isPseudo('re')) oe['pseudophakia_re'] = {
        ...
        if (isPseudo('le')) oe['pseudophakia_le'] = {
      ...
      case 9: // Diagnosis
        examData['diagnoses'] = _selectedDiagnosisIds.toList();
      case 11: // Advice
        examData['advice'] = _adviceCtrl.text.trim();
    }
    ...
    // Medicine step sends medicines at top level; backend syncs prescriptions
    if (step == 10) {
      payload['medicines'] = _medicines
          .where((m) => m.medicineId != null || m.medicineName.isNotEmpty)
          .map((m) => m.toJson())
          .toList();
    }
    return payload;
          ...
          return GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                color: isActive ? AppColors.teal : Colors.white.withValues(alpha: 0.15),
                borderRadius: BorderRadius.circular(AppRadius.xl),
                border: Border.all(color: isActive ? AppColors.teal : Colors.white.withValues(alpha: 0.3)),
              ),
              child: Text(
                '${i + 1}. ${_stepNames[i]}',
                style: TextStyle(
      ...
      child: Row(children: [
        ...
        if (!isFirst) ...[
          Expanded(
            child: OutlinedButton.icon(
              ...
              style: OutlinedButton.styleFrom(foregroundColor: AppColors.teal, side: const BorderSide(color: AppColors.teal), padding: const EdgeInsets.symmetric(vertical: 10)),
            ),
          ),
        ...
        Expanded(
          ...
          child: ElevatedButton.icon(
            ...
            style: ElevatedButton.styleFrom(backgroundColor: AppColors.teal, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 10)),
          ),
        ),
        // ── Next ─────────────────────────────────────────────────────────────
        if (!isLast) ...[
          ...
          Expanded(
            child: ElevatedButton.icon(
              ...
              style: ElevatedButton.styleFrom(
                backgroundColor: AppColors.teal.withValues(alpha: 0.75),
  ...
  Future<void> _toggleDiagnosisFav(ExamMasterItem d) async {
    try {
      final newFav = await SimpleMasterService.instance.toggleFavourite('masters/detail/diagnoses', d.id);
      final idx = _diagnosisMasters.indexWhere((e) => e.id == d.id);
      if (idx >= 0 && mounted) {
        setState(() {
          _diagnosisMasters[idx] = ExamMasterItem(id: d.id, value: d.value, isFavourite: newFav);
        });
      }
    } catch (e) {
      debugPrint('[Diagnosis] toggleFav error: $e');
    }
  }

  // ── Advice helpers ────────────────────────────────────────────────────────────

  void _appendAdviceText(String text) {
    final existing = _adviceCtrl.text.trimRight();
    _adviceCtrl.text = existing.isEmpty ? text : '$existing\n$text';
    _adviceCtrl.selection = TextSelection.fromPosition(
      TextPosition(offset: _adviceCtrl.text.length),
    );
  }

  void _showAdviceMoreSheet() {
    String query = '';
    ...
    showModalBottomSheet<void>(
      ...
      builder: (bsCtx) => StatefulBuilder(builder: (_, bsSs) {
        final filtered = masterList
            .where((i) => query.isEmpty || i.value.toLowerCase().contains(query.toLowerCase()))
            .toList();
        ...
        Future<void> doToggle(ExamMasterItem item) async {
          try {
            final newFav = await SimpleMasterService.instance.toggleFavourite('masters/detail/advices', item.id);
            bsSs(() {
            ...
            if (mounted) setState(() { _adviceMasters = List<ExamMasterItem>.from(masterList); });
          } catch (_) {}
        }
        ...
        Future<void> doCreate() async {
          ...
          try {
            ...
            final newItem = ExamMasterItem(id: created.id, value: created.value, isFavourite: created.isFavourite);
            bsSs(() { masterList.add(newItem); addCtrl.clear(); });
            if (mounted) {
              setState(() { _adviceMasters.add(newItem); });
              _appendAdviceText(newItem.value);
            }
          } catch (_) {}
        }
        ...
        Widget itemTile(ExamMasterItem item) => ListTile(
          ...
          onTap: () { _appendAdviceText(item.value); if (mounted) setState(() {}); },
        );

        return DraggableScrollableSheet(
          expand: false, initialChildSize: 0.65, maxChildSize: 0.92, minChildSize: 0.3,
          builder: (_, sc) => Column(children: [
            ...
            Padding(padding: EdgeInsets.fromLTRB(16, 0, 16, 8), child: Row(children: [
              Expanded(child: Text('All Advice', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: AppColors.primary))),
            ])),
              ...
              GestureDetector(
                onTap: () => showDialog<void>(
                  ...
                  builder: (dCtx) => AlertDialog(
                    title: const Text('New Advice', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700)),
                    content: TextField(
                      controller: addCtrl, autofocus: true, maxLines: 3,
                      decoration: const InputDecoration(hintText: 'Type advice text…', isDense: true),
                    ),
                    actions: [
                      TextButton(onPressed: () => Navigator.pop(dCtx), child: const Text('Cancel')),
                      ElevatedButton(
                        onPressed: () { Navigator.pop(dCtx); doCreate(); },
                        style: ElevatedButton.styleFrom(backgroundColor: AppColors.primary),
                        child: const Text('Save & Append', style: TextStyle(fontSize: 12, color: Colors.white)),
                      ),
                    ],
                  ),
                ),
            ...
            Expanded(child: ListView(controller: sc, children: [
              ...
              if (favs.isEmpty && others.isEmpty)
                Padding(padding: const EdgeInsets.all(24), child: Text(
                    query.isEmpty ? 'No advice items yet — tap Add to create one' : 'No match — tap Add to create',
  ...
  Widget _buildDilateStep() => ListView(padding: const EdgeInsets.fromLTRB(16, 12, 16, 100), children: [
    ...
    Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          ChoiceChip(label: const Text('Yes'), selected: _dilate == 'Yes', selectedColor: AppColors.teal,
          ...
          const SizedBox(width: 8),
          ChoiceChip(label: const Text('No'), selected: _dilate == 'No', selectedColor: AppColors.teal,
  ...
  Widget _buildMedicineStep() {
    ...
    return ListView(padding: const EdgeInsets.fromLTRB(16, 12, 16, 100), children: [
      ...
      Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            const Icon(Icons.medication_rounded, size: 16, color: Colors.white),
            const SizedBox(width: 6),
            const Text('Medicines (Rx)', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w800, color: Colors.white)),
            const Spacer(),
            GestureDetector(
              onTap: () => setState(() => _medicines.add(_PrescRow())),
              child: Container(
                padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
                decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(AppRadius.sm)),
                child: const Row(mainAxisSize: MainAxisSize.min, children: [
                  Icon(Icons.add_rounded, size: 14, color: Colors.white),
                  SizedBox(width: 4),
          ...
          if (groups.isNotEmpty) ...[
            ...
            DropdownButtonFormField<int>(
              value: null,
              isExpanded: true,
              icon: const Icon(Icons.expand_more, size: 14, color: Colors.white70),
              style: const TextStyle(fontSize: 12, color: Colors.black87),
              hint: const Row(children: [
                Icon(Icons.playlist_add_rounded, size: 14, color: Colors.white70),
                SizedBox(width: 6),
                Text('Load a medicine group…', style: TextStyle(fontSize: 12, color: Colors.white70)),
              ]),
              ...
              decoration: InputDecoration(
                ...
                border: OutlineInputBorder(borderRadius: BorderRadius.circular(AppRadius.sm), borderSide: BorderSide(color: Colors.white.withValues(alpha: 0.3))),
                enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(AppRadius.sm), borderSide: BorderSide(color: Colors.white.withValues(alpha: 0.3))),
                focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(AppRadius.sm), borderSide: const BorderSide(color: Colors.white)),
              ),
      ...
      const SizedBox(height: 8),
      if (_medicines.isEmpty) _emptyHint('Tap + to add a prescription')
      else ..._medicines.asMap().entries.map((e) => _prescRow(e.key, e.value, dosages, routes)),
    ]);
  }

  Widget _buildAdviceStep() {
      ...
      Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            const SizedBox(width: 6),
            Expanded(child: Text('QUICK ADD',
                style: TextStyle(fontSize: 10, fontWeight: FontWeight.w800,
                    letterSpacing: 0.8, color: AppColors.primary.withValues(alpha: 0.55)))),
            GestureDetector(
              onTap: _showAdviceMoreSheet,
              child: Container(
                ...
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  Icon(Icons.grid_view_rounded, size: 12, color: AppColors.primary.withValues(alpha: 0.6)),
                  const SizedBox(width: 4),
                  Text('More', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: AppColors.primary.withValues(alpha: 0.7))),
                ]),
              ),
            ),
          ]),
          ...
          if (favAdvices.isNotEmpty)
            Wrap(spacing: 6, runSpacing: 6, children: favAdvices.map((a) {
              return GestureDetector(
                onTap: () { _appendAdviceText(a.value); setState(() {}); },
                child: Container(
                  padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
                  decoration: BoxDecoration(
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    ...
                    const SizedBox(width: 4),
                    Text(a.value, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: Color(0xFF92400E))),
                  ]),
                ),
              );
            }).toList())
          else
            Text('No favourites yet — tap More → ★ to mark',
                style: TextStyle(fontSize: 11, color: AppColors.primary.withValues(alpha: 0.4), fontStyle: FontStyle.italic)),
        ]),
      ),
      const SizedBox(height: 12),

      // ── Advice textarea with char counter ─────────────────────────────────────
      ...
      ValueListenableBuilder<TextEditingValue>(
        ...
        builder: (_, val, __) => Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
          TextFormField(
            ...
            decoration: _examDeco('Clinical advice, post-operative care, follow-up instructions...')
                .copyWith(counterText: ''),
          ),
          const SizedBox(height: 2),
          Text('${val.text.length} / 2000',
              style: TextStyle(fontSize: 10,
                  color: val.text.length > 1800 ? Colors.red : AppColors.primary.withValues(alpha: 0.4))),
        ]),
      ),
      const SizedBox(height: 12),
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        body: Column(children: [
          _buildAppBar(),
          if (_prefillSource == 'primary') _primaryDataBanner(),
  ...
  Widget _primaryDataBanner() => Container(
    padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
    color: Colors.amber.shade50,
    child: Row(children: [
      Icon(Icons.info_outline_rounded, size: 16, color: Colors.amber.shade800),
      const SizedBox(width: 8),
      const Expanded(child: Text('Pre-filled from Primary Examination — review and update', style: TextStyle(fontSize: 11, color: Colors.black87))),
    ]),
  );
  ...
  Widget _buildAppBar() {
    ...
    return Container(
      decoration: const BoxDecoration(gradient: LinearGradient(colors: [AppColors.teal, Color(0xFF16A085)], begin: Alignment.topLeft, end: Alignment.bottomRight)),
      child: SafeArea(bottom: false, child: Padding(
        ...
        child: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Secondary Examination', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w800)),
            const SizedBox(height: 2),
          ...
          if (PermissionService.instance.can(Perm.otSurgeryRecommend))
            IconButton(
              ...
              onPressed: () => showRecommendSurgerySheet(context, patient: widget.patient, diagnosisHint: _diagnosisHintText()),
            ),
        ]),
      )),
    );
  ...
  Widget _coRow(int idx, _CoRow row) {
    // Web eye values: '' (dash), RE, LE, Both, OU — we store '-' locally for ''
  ...
  // NS (near SPH) chip + ADD text field stacked
  ...
  Widget _stNsCell(String eye) {
    ...
    return Expanded(child: Padding(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        GestureDetector(
          onTap: () async {
            // Pick ADD value; NS is auto-calculated after
  ...
  Widget _stCheck(String label, bool val, void Function(bool?) onChanged) {
    return Row(mainAxisSize: MainAxisSize.min, children: [
      Checkbox(value: val, onChanged: onChanged, activeColor: AppColors.teal, materialTapTargetSize: MaterialTapTargetSize.shrinkWrap),
  ...
  Widget _buildDiagnosisSection() {
    final filtered = _diagnosisSearch.isEmpty
        ? _diagnosisMasters
        : _diagnosisMasters.where((d) => d.value.toLowerCase().contains(_diagnosisSearch.toLowerCase())).toList();
    final sorted = [
      ...filtered.where((d) => d.isFavourite),
      ...filtered.where((d) => !d.isFavourite),
    ];
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        Text('Diagnoses', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w800, color: AppColors.primary)),
        if (_selectedDiagnosisIds.isNotEmpty) ...[
          const SizedBox(width: 8),
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
            decoration: BoxDecoration(color: AppColors.teal, borderRadius: BorderRadius.circular(AppRadius.md)),
            child: Text('${_selectedDiagnosisIds.length} selected',
                style: const TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w600)),
          ),
          const Spacer(),
          GestureDetector(
            onTap: () => setState(() => _selectedDiagnosisIds.clear()),
            child: Text('Clear all', style: TextStyle(fontSize: 11, color: Colors.red.shade600, fontWeight: FontWeight.w600)),
          ),
        ],
      ]),
      const SizedBox(height: 8),
      TextFormField(
        initialValue: _diagnosisSearch,
        onChanged: (v) => setState(() => _diagnosisSearch = v),
        style: const TextStyle(fontSize: 13),
        decoration: InputDecoration(
          hintText: 'Search diagnoses…',
          hintStyle: TextStyle(fontSize: 12, color: AppColors.primary.withValues(alpha: 0.35)),
          prefixIcon: Icon(Icons.search_rounded, size: 18, color: AppColors.primary.withValues(alpha: 0.5)),
      ...
      if (sorted.isEmpty)
        Padding(
          ...
          child: Text(
            _diagnosisSearch.isEmpty ? 'No diagnoses available' : 'No results for "$_diagnosisSearch"',
            style: TextStyle(fontSize: 12, color: AppColors.primary.withValues(alpha: 0.4), fontStyle: FontStyle.italic),
          ),
        )
      else
        Wrap(
          ...
          children: sorted.map((d) {
            final selected = _selectedDiagnosisIds.contains(d.id);
            return Container(
              decoration: BoxDecoration(
                color: selected ? AppColors.teal : Colors.white,
                borderRadius: BorderRadius.circular(AppRadius.xl),
                border: Border.all(color: selected ? AppColors.teal : AppColors.primary.withValues(alpha: 0.2)),
              ),
              child: IntrinsicHeight(
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  // ☆/★ favourite toggle
                  GestureDetector(
                    onTap: () => _toggleDiagnosisFav(d),
                    child: Container(
                      padding: const EdgeInsets.only(left: 8, top: 6, bottom: 6, right: 4),
                      child: Icon(
                        d.isFavourite ? Icons.star_rounded : Icons.star_border_rounded,
                        size: 13,
                        color: d.isFavourite
                            ? Colors.amber.shade600
                            : (selected ? Colors.white54 : AppColors.primary.withValues(alpha: 0.3)),
                      ),
                    ),
                  ),
                  Container(width: 0.5, color: selected ? Colors.white24 : AppColors.primary.withValues(alpha: 0.1)),
                  // Main selection tap area
                  GestureDetector(
                    onTap: () => setState(() {
                      if (selected) _selectedDiagnosisIds.remove(d.id);
                      else _selectedDiagnosisIds.add(d.id);
                    }),
                    child: Padding(
                      padding: const EdgeInsets.only(left: 8, right: 10, top: 6, bottom: 6),
                      child: Row(mainAxisSize: MainAxisSize.min, children: [
                        if (selected) const Icon(Icons.check_rounded, size: 13, color: Colors.white),
                        if (selected) const SizedBox(width: 3),
                        Text(d.value, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: selected ? Colors.white : AppColors.primary)),
                      ]),
                    ),
                  ),
                ]),
  ...
  Widget _prescRow(int idx, _PrescRow row, List<MedMasterItem> dosages, List<MedMasterItem> routes) {
    return Container(
      margin: const EdgeInsets.only(bottom: 8), padding: const EdgeInsets.all(10),
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(AppRadius.md), border: Border.all(color: AppColors.primary.withValues(alpha: 0.12))),
      child: Column(children: [
        Row(children: [
          Expanded(child: _MedicineSearchField(initialName: row.medicineName, onSelected: (med) => setState(() {
          ...
          const SizedBox(width: 6),
          GestureDetector(onTap: () => setState(() => _medicines.removeAt(idx)), child: const Icon(Icons.remove_circle_rounded, color: Color(0xFFDC3545), size: 22)),
        ]),
        const SizedBox(height: 6),
        Row(children: [
          Expanded(flex: 2, child: _miniDropdownItems<int?>(value: row.dosageId, hint: 'Dosage',
              items: [const DropdownMenuItem(value: null, child: Text('—')), ...dosages.map((d) => DropdownMenuItem(value: d.id, child: Text(d.name, overflow: TextOverflow.ellipsis)))],
              onChanged: (v) => setState(() => row.dosageId = v))),
          const SizedBox(width: 6),
          Expanded(flex: 2, child: _miniDropdownItems<int?>(value: row.routeId, hint: 'Route',
              items: [const DropdownMenuItem(value: null, child: Text('—')), ...routes.map((r) => DropdownMenuItem(value: r.id, child: Text(r.name, overflow: TextOverflow.ellipsis)))],
              onChanged: (v) => setState(() => row.routeId = v))),
          const SizedBox(width: 6),
          SizedBox(width: 68, child: _miniField(initialValue: row.duration, hint: 'Days', inputType: TextInputType.number, onChanged: (v) => row.duration = v)),
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border(
                ...
                right: BorderSide(color: AppColors.primary.withValues(alpha: 0.15)),
                bottom: BorderSide(color: AppColors.primary.withValues(alpha: 0.15)),
              ),
              borderRadius: const BorderRadius.only(topRight: Radius.circular(AppRadius.sm), bottomRight: Radius.circular(AppRadius.sm)),
            ),
            child: Text('D', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: AppColors.primary)),
          ),
          const SizedBox(width: 6),
          SizedBox(width: 46, child: _miniField(initialValue: row.quantity.toString(), hint: 'Qty', inputType: TextInputType.number, onChanged: (v) => row.quantity = int.tryParse(v) ?? 1)),
        ]),
      ]),
    );
  }
  ...
  Widget _addBtn(VoidCallback onTap) => GestureDetector(onTap: onTap,
        child: Container(padding: const EdgeInsets.all(4),
            decoration: BoxDecoration(color: AppColors.teal.withValues(alpha: 0.12), borderRadius: BorderRadius.circular(AppRadius.xl)),
  ...
  Widget _miniDropdownItems<T>({required T value, required List<DropdownMenuItem<T>> items, required void Function(T?) onChanged, required String hint}) {
    return DropdownButtonFormField<T>(initialValue: value, isExpanded: true, icon: const Icon(Icons.expand_more, size: 14),
        style: const TextStyle(fontSize: 11, color: Colors.black87), hint: Text(hint, style: const TextStyle(fontSize: 11)),
        decoration: InputDecoration(contentPadding: const EdgeInsets.symmetric(vertical: 8, horizontal: 8), filled: true, fillColor: Colors.white,
          border: OutlineInputBorder(borderRadius: BorderRadius.circular(AppRadius.sm), borderSide: BorderSide(color: AppColors.primary.withValues(alpha: 0.15))),
          enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(AppRadius.sm), borderSide: BorderSide(color: AppColors.primary.withValues(alpha: 0.15))),
          focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(AppRadius.sm), borderSide: BorderSide(color: AppColors.primary))),
...
class _MedicineSearchFieldState extends State<_MedicineSearchField> {
  ...
  Widget _suggestionTile(MedItem m) {
    final meta = [
      if (m.medicineTypeName != null) m.medicineTypeName!,
      if (m.duration != null) '${m.duration} days',
    ].join(' · ');
    return InkWell(
      ...
      child: Padding(
        ...
        child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if (meta.isNotEmpty)
              Text(meta, style: TextStyle(fontSize: 10, color: AppColors.primary.withValues(alpha: 0.55))),
          ])),
          if (m.dosageText != null)
            Container(
              margin: const EdgeInsets.only(left: 6, top: 1),
              padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 2),
              decoration: BoxDecoration(color: const Color(0xFFE0F2FE), borderRadius: BorderRadius.circular(AppRadius.xs)),
              child: Text(m.dosageText!, style: const TextStyle(fontSize: 10, color: Color(0xFF0369A1), fontWeight: FontWeight.w500)),
            ),
        ]),
      ),
    );
    ...
    return Column(children: [
      ...
      if (_showSuggestions)
        Container(
          ...
          child: Column(children: _results.take(6).map((m) => _suggestionTile(m)).toList()),
        ),
    ]);
  }
}

...
class _PatientFormScreenState extends State<PatientFormScreen>
  ...
  late final AnimationController _formEnter;
  late final CurvedAnimation _formEnterAnim;
  ...
  void initState() {
    super.initState();
    _formEnter = AnimationController(vsync: this, duration: const Duration(milliseconds: 320));
    _formEnterAnim = CurvedAnimation(parent: _formEnter, curve: Curves.easeOut);
    _initDate();
    _loadMasters();
  ...
  void _initDate() {
    ...
    } else if (widget.mode == PatientFormMode.edit &&
        widget.patient?.appointmentDate != null) {
      _appointmentDate =
          DateTime.tryParse(widget.patient!.appointmentDate!) ?? DateTime.now();
    }
  ...
  void _onContactChanged(String val) {
    ...
    _contactDebounce = Timer(const Duration(milliseconds: 500), () async {
      try {
        final results =
            await PatientService.instance.searchByContact(val);
        if (mounted) {
          setState(() {
  ...
  Future<void> _pickDate() async {
    final picked = await showDatePicker(
      ...
      firstDate: widget.mode == PatientFormMode.edit
          ? DateTime(2020)
          : DateTime.now(),
  ...
  Future<void> _addCity() async {
    ...
    if (cityCtrl.text.trim().isEmpty) return;

    try {
      final loc = await MastersService.instance.addLocation(
      ...
      if (mounted) {
        setState(() {
          _masters = MastersData(
            cases: _masters!.cases,
            doctors: _masters!.doctors,
            locations: [..._masters!.locations, loc],
            slots: _masters!.slots,
            referrers: _masters!.referrers,
          );
  ...
  Future<void> _submit() async {
    ...
    setState(() { _submitting = true; _submitSuccess = false; });
    ...
    try {
      ...
      if (isEdit) {
        // case_fee is editable in edit
        data['case_fee'] =
            double.tryParse(_caseFeeCtrl.text.trim()) ?? 0;
        data['case_id'] = _selectedCaseId;
        await PatientService.instance
            .updatePatient(widget.patient!.id, data);
      } else if (isPhone) {
        await PatientService.instance.registerPhone(data);
      } else {
        await PatientService.instance.registerWalkIn(data);
      }
          ...
          Expanded(
            child: _mastersLoading
                ...
                : _mastersError != null
                    ? _buildMastersError()
                    : FadeTransition(
                        opacity: _formEnterAnim,
                        child: SlideTransition(
                          position: Tween<Offset>(
                            begin: const Offset(0, 0.03),
                            end: Offset.zero,
                          ).animate(_formEnterAnim),
                          child: _buildForm(),
                        ),
                      ),
          ),
  ...
  Widget _buildAppBar() {
    final (title, subtitle, icon) = switch (widget.mode) {
      PatientFormMode.addWalkIn => (
          'Register New Patient',
          'Capture patient information and appointment details',
          Icons.person_add_rounded,
        ),
      PatientFormMode.addPhone => (
          'Register Phone Appointment',
          'Capture caller details and appointment information',
  ...
  Widget _buildForm() {
    ...
    return Form(
      ...
      child: ListView(
        ...
        children: [
          // ── Warning banner for edit (exam done) ─────────────────────
          if (isEdit && _examDoneWarning && !_warnDismissed)
            _buildWarnBanner(),

          // ── MRD (readonly) ──────────────────────────────────────────
          _sectionLabel('MRD Number'),
          _readonlyField(_mrdCtrl, 'MRD No.', Icons.badge_outlined),
          ...
          _contactField(),
          if (_showSuggestions) _suggestionDropdown(),

          // ── WhatsApp ─────────────────────────────────────────────────
          _sectionLabel('WhatsApp Number'),
          _textField(
            _whatsappCtrl,
            'WhatsApp No (same if blank)',
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    _textField(_firstCtrl, 'First Name',
                        required: true),
                  ],
                ),
              ),
          ...
          // ── Case Type + Fee (walk-in and edit only) ──────────────────
          if (!isPhone) ...[
            ...
            _textField(
              ...
              required: true,
              readOnly: widget.mode == PatientFormMode.addWalkIn,
              hintOverride: widget.mode == PatientFormMode.addWalkIn
                  ? 'Auto-filled from Case Type'
                  : null,
            ),
          ],
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    _readonlyField(
                        _districtCtrl, 'District', Icons.location_city_outlined),
                  ],
                ),
              ),
              const SizedBox(width: 12),
  ...
  InputDecoration _inputDeco(String hint,
    ...
    return InputDecoration(
      hintText: hint,
      hintStyle: TextStyle(
          fontSize: 13, color: AppColors.primary.withValues(alpha: 0.4)),
      prefixIcon: prefix != null
          ? Icon(prefix, size: 18, color: AppColors.primary.withValues(alpha: 0.5))
          : null,
      filled: true,
      fillColor: readOnly
  ...
  Widget _textField(
    ...
    return TextFormField(
      ...
      inputFormatters: inputType == TextInputType.number
          ...
          : null,
      decoration: _inputDeco(hintOverride ?? hint,
              readOnly: readOnly)
          .copyWith(
        counterText: '',
  ...
  Widget _readonlyField(
    ...
    return TextFormField(
      ...
      readOnly: true,
      style: TextStyle(
          color: AppColors.primary.withValues(alpha: 0.65), fontWeight: FontWeight.w500),
    ...
    return GestureDetector(
      ...
      child: AbsorbPointer(
        child: TextFormField(
          ...
          validator: (v) =>
              (v == null || v.isEmpty) ? 'Appointment date required' : null,
        ),
      ),
    );
  }
  ...
  Widget _contactField() {
    return TextFormField(
      ...
      decoration: _inputDeco('Contact number (e.g. +91XXXXXXXXXX)',
              ...
              child: Row(
                children: [
                  ...
                  Container(
                    ...
                    child: Text(
                      isLocal ? 'Own' : s.hospitalName ?? 'Partner',
                      style: TextStyle(
                          fontSize: 10,
                          fontWeight: FontWeight.w700,
                          color: isLocal
  ...
  Widget _buildDropdown<T>({
    ...
    return DropdownButtonFormField<T>(
      ...
      icon:
          Icon(Icons.expand_more_rounded, color: AppColors.primary.withValues(alpha: 0.6)),
          ...
          Expanded(
            child: Column(
              ...
              children: const [
                ...
                SizedBox(height: 2),
                Text(
                    'Primary examination has already been done. Changes will not affect examination data.',
                    style:
                        TextStyle(fontSize: 11, color: Color(0xFF784212))),
              ],
            ),
          ),
          IconButton(
            icon: const Icon(Icons.close_rounded,
                size: 16, color: Color(0xFF784212)),
            padding: EdgeInsets.zero,
            constraints: const BoxConstraints(),
            onPressed: () => setState(() => _warnDismissed = true),
          ),
        ],
      ),
    );
  ...
  Widget _submitButton() {
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: (_submitting || _submitSuccess) ? null : _submit,
        style: ElevatedButton.styleFrom(
          backgroundColor: _submitSuccess ? Colors.green.shade600 : AppColors.primary,
          foregroundColor: Colors.white,
          disabledBackgroundColor: _submitSuccess
        ...
        child: AnimatedSwitcher(
          ...
          child: _submitSuccess
              ...
              : _submitting
                  ? const SizedBox(
                      width: 18, height: 18, key: ValueKey('spin'),
                      child: CircularProgressIndicator(
                          color: Colors.white, strokeWidth: 2))
                  : Row(key: const ValueKey('txt'),
                      mainAxisSize: MainAxisSize.min,
                      children: [

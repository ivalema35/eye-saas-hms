...
class _OtCounsellingFormScreenState extends State<OtCounsellingFormScreen> {
  ...
  // Lens Cost is a dropdown of OtPackageMaster options on web (each option
  // carries room/package-name/4-charge data that auto-fills the rest of the
  // form on selection) — not free text. See OT_WEB_PARITY_FIX_PRD.md §3.
      ...
      if (mounted) {
        setState(() {
          ...
          if (c != null) {
            ...
            // Best-effort: pre-select the matching package option in the
            // Lens Cost dropdown if one has the same cost + room category.
  ...
  /// Selecting a Lens Cost option auto-fills package name, room category,
  /// and all 4 charge fields from that package — matches web's `<option
  /// data-*>` autofill exactly (no separate lookup call; the earlier
  /// `package-lookup` endpoint is confirmed unused by the real web UI).
      ...
      Expanded(
        child: ListView(
          ...
          children: [
            ...
            SwitchListTile(value: _surgeryTypeConfirmed, onChanged: (v) => setState(() => _surgeryTypeConfirmed = v), title: const Text('Surgery type confirmed', style: TextStyle(fontSize: 13)), contentPadding: EdgeInsets.zero, activeThumbColor: AppColors.teal),
            ...
            Container(
              ...
              child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                ...
                Text('₹${(_lensCost ?? 0).toStringAsFixed(2)}', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: AppColors.primary)),
              ]),
            ),
            const SizedBox(height: 16),
            ...
            SwitchListTile(value: _bloodReportsVerified, onChanged: (v) => setState(() => _bloodReportsVerified = v), title: const Text('Blood reports verified', style: TextStyle(fontSize: 13)), contentPadding: EdgeInsets.zero, activeThumbColor: AppColors.teal),
            SwitchListTile(value: _bloodReportsNormal, onChanged: (v) => setState(() => _bloodReportsNormal = v), title: const Text('Blood reports normal', style: TextStyle(fontSize: 13)), contentPadding: EdgeInsets.zero, activeThumbColor: AppColors.teal),
            const SizedBox(height: 10),

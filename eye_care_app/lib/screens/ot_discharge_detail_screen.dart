...
/// Round 3 Phase 6 — Discharge detail: generate invoice + 8 individual
/// print documents + Print All. `discharge-bundle` is a URL manifest, not
/// a merged PDF (see build PRD §10 gotcha) — "Print All" downloads/opens
/// each document from that manifest in sequence.
  ...
  Widget _buildBody() {
    return ListView(
      ...
      children: [
        ...
        Text('"Print All" opens each document individually — the backend has no single merged PDF (matches web behavior).', style: TextStyle(fontSize: 11, color: AppColors.textSecondary), textAlign: TextAlign.center),
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        InkWell(onTap: _pickFollowUp, child: InputDecorator(decoration: const InputDecoration(labelText: 'Follow-up Date (optional, defaults +7 days)'), child: Text(_followUpDate == null ? 'Select' : '${_followUpDate!.year}-${_followUpDate!.month.toString().padLeft(2, '0')}-${_followUpDate!.day.toString().padLeft(2, '0')}'))),
      ]),
      actions: [

...
class OtTypeMasterScreen extends StatefulWidget {
  ...
  const OtTypeMasterScreen({super.key, this.accentColor = AppColors.green});
...
class _OtTypeMasterScreenState extends State<OtTypeMasterScreen> {
  ...
  List<OtTypeMasterItem> get _filtered =>
  ...
  void _openSheet({OtTypeMasterItem? item}) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.white,
      shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(24))),
      builder: (_) => _OtTypeSheet(
        item: item,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('OT Types', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w800, letterSpacing: -0.2)),
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: TextField(
          ...
          decoration: const InputDecoration(hintText: 'Search OT types...', hintStyle: TextStyle(color: Color(0xFF94A3B8), fontSize: 13), prefixIcon: Icon(Icons.search_rounded, color: Color(0xFF94A3B8), size: 20), border: InputBorder.none, contentPadding: EdgeInsets.symmetric(horizontal: 12, vertical: 12)),
        ),
      ),
    );
  }
      ...
      return ListView(children: [
        ...
        Center(
          child: Column(children: [
            Icon(Icons.label_rounded, size: 56, color: AppColors.primary.withValues(alpha: 0.15)),
            const SizedBox(height: 12),
            Text(_query.isNotEmpty ? 'No results for "$_query"' : 'No OT types yet.\nTap + to add one.', textAlign: TextAlign.center, style: const TextStyle(color: Color(0xFF94A3B8), fontSize: 14, fontWeight: FontWeight.w500)),
          ]),
        ),
      ]);
    }
...
class _OtTypeSheet extends StatefulWidget {
  final OtTypeMasterItem? item;
  final Color accentColor;
  final Future<void> Function(String name) onSave;

  const _OtTypeSheet({required this.item, required this.accentColor, required this.onSave});

  @override
  State<_OtTypeSheet> createState() => _OtTypeSheetState();
}

class _OtTypeSheetState extends State<_OtTypeSheet> {
      ...
      child: Column(
        ...
        children: [
          ...
          Padding(
            ...
            child: Row(children: [
              Container(padding: const EdgeInsets.all(7), decoration: BoxDecoration(color: widget.accentColor.withValues(alpha: 0.12), borderRadius: BorderRadius.circular(10)), child: Icon(Icons.label_rounded, size: 16, color: widget.accentColor)),
              const SizedBox(width: 10),
              Expanded(child: Text(widget.item == null ? 'Add OT Type' : 'Edit OT Type', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: AppColors.darkNavy))),

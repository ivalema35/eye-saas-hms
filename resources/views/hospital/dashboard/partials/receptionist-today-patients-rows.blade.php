@forelse($receptionistTodayPatients as $index => $patient)
    @php
        $isOt = ($patient->source ?? 'patient') === 'ot_appointment'
            || strtolower((string) ($patient->type ?? '')) === 'ot';
    @endphp
    ...
    @else
        @php
            ...
            $initials = strtoupper(substr((string) $patient->first_name, 0, 1) . substr((string) $patient->last_name, 0, 1));
            $isPhone = strtolower((string) $patient->type) === 'phone';
        @endphp
        <tr>
            <td style="color:#94a3b8; font-size:12px; font-weight:600;">{{ $index + 1 }}</td>
            <td><span class="tap-mrd">{{ $patient->patient_code }}</span></td>
            <td>
                <div class="tap-patient-cell">
                    ...
                    <div>
                        <div class="tap-name">{{ $patient->full_name }}</div>
                        <span class="tap-type-pill {{ $isPhone ? 'tap-type-phone' : 'tap-type-walkin' }}">
                            <i class="bi bi-{{ $isPhone ? 'phone' : 'person-walking' }}" style="font-size:9px;"></i>
                            {{ $isPhone ? 'Phone' : 'Walkin' }}
                        </span>
                    </div>
                </div>
            </td>
            <td class="tap-meta">
                <strong>{{ $patient->cityName ?: '—' }}</strong><br>
                <span>{{ $patient->age }}y / {{ ucfirst((string) $patient->gender) }}</span>
            </td>
            <td class="tap-slot">
                @if($patient->contact_no)
                    {{ $patient->contact_no }}

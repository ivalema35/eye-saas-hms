...
@section('content')
    <style>
        ...
        .custom-partner-card {
            background: #fff;
            border-radius: 16px;
            box-shadow: 0 10px 25px -5px rgba(27, 79, 114, .1);
    ...
    <div class="partner-page-wrap">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 pb-2 border-bottom">
            <div>
                ...
                <small class="text-muted">
                    <i
                        class="bi bi-geo-alt me-1"></i>{{ $partnerTenant->city ?? '' }}{{ $partnerTenant->state ? ', ' . $partnerTenant->state : '' }}
                </small>
            </div>
        ...
        {{-- Filter --}}
        <form method="GET"
            action="{{ route('hospital.partner.history', ['slug' => $slug, 'partnerTenantId' => $partnerTenant->id]) }}"

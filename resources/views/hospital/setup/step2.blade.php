...
@section('wizard-content')
    ...
    <form method="POST" action="{{ route('hospital.setup.store', ['slug' => $slug, 'step' => $step]) }}">
        ...
        <div class="wizard-card-body">
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:1rem">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="contact" name="contact" class="form-input" value="{{ old('contact') }}"
                        data-intl-phone placeholder="+919876543210">
                    @error('contact')
                    <div class="form-error">{{ $message }}</div> @enderror
                </div>
            </div>
            ...
            <!-- <div class="hms-form-group user-doctor-only" style="display:none">
                            <label>Doctor Prefix <span style="font-size:.75rem;color:#64748B;font-weight:400">(2–5
                                    letters)</span></label>
                            <input type="text" name="doctor_prefix" id="user-doctor-prefix" maxlength="5" placeholder="e.g. JP"
                                style="text-transform:uppercase" class="hms-input @error('doctor_prefix') is-invalid @enderror">
            ...
            <div class="form-group">
                <label class="form-label" for="doctor_prefix">Doctor Prefix <span style="color:#C0392B">*</span></label>
                <input type="text" id="doctor_prefix" name="doctor_prefix" class="form-input"
                    value="{{ old('doctor_prefix') }}" required maxlength="5" placeholder="e.g. JP"
                    style="text-transform:uppercase">
                <div style="font-size:.72rem;color:#94A3B8;margin-top:.2rem">Daily serial format: <strong>JP-001,
                        JP-002…</strong></div>
                @error('doctor_prefix')
                <div class="form-error">{{ $message }}</div> @enderror
            </div>
        ...
        <div class="wizard-actions">
            <button type="button" class="btn-wizard btn-wizard-skip"
    ...
    <form id="skip-form" method="POST" action="{{ route('hospital.setup.skip', ['slug' => $slug, 'step' => $step]) }}"
        style="display:none">
        @csrf
    </form>
@endsection

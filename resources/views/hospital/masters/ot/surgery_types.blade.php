...
@section('content')
    <div class="ot-master-page">
        <div class="ot-master-toolbar">
            ...
            @haspermission('ot_surgery_type_add')
            <button type="button" class="btn btn-primary ot-master-add-btn" data-bs-toggle="modal"
                data-bs-target="#surgeryFormModal" onclick="resetForm()">
                <i class="bi bi-plus-lg me-1"></i> Add Surgery Type
            </button>
            @endhaspermission
        </div>
                                ...
                                <tr>
                                    ...
                                    <td class="text-end pe-4">
                                        <div class="btn-group shadow-sm rounded-3" role="group">
                                            @haspermission('ot_surgery_type_edit')
                                            ...
                                            @endhaspermission
                                            @haspermission('ot_surgery_type_delete')
                                            <form method="POST" class="d-inline"
                                                action="{{ route('hospital.masters.ot.surgery-types.destroy', ['slug' => $slug, 'id' => $record->id]) }}"
                                                onsubmit="return confirm('Delete this surgery type?');">
                                                @csrf
                                                @method('DELETE')
        ...
        <div class="modal fade ot-master-modal" id="surgeryFormModal" tabindex="-1" aria-hidden="true">
                ...
                <div class="modal-content border-0">
                    <div class="modal-header border-0 pb-0">
                        <div class="d-flex align-items-center">
                            ...
                            <h5 class="modal-title fw-bold mb-0" id="formTitle">Add OT Surgery Type</h5>
                        </div>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>

                    <form id="surgeryForm" method="POST"
                        action="{{ route('hospital.masters.ot.surgery-types.store', ['slug' => $slug]) }}">
                        @csrf
                        <input type="hidden" name="_method" id="formMethod" value="POST">

                        <div class="modal-body">
                            <div class="mb-3">
                                <label class="form-label text-muted small fw-bold text-uppercase">OT Type</label>
                                <select name="ot_type_id" id="ot_type_id" class="form-select form-select-lg ot-master-input"
                                    required>
                            ...
                            <div class="mb-3">
                                <label class="form-label text-muted small fw-bold text-uppercase">Surgery Name</label>
                                <input type="text" name="surgery_name" id="surgery_name"
                                    class="form-control form-control-lg ot-master-input" value="{{ old('surgery_name') }}"
                                    required>
                            </div>
                        </div>
                        ...
                        <div class="modal-footer border-0 gap-2">
                            ...
                            <button class="btn btn-primary fw-semibold rounded-3" type="submit" id="submitBtn">Save Surgery
                                Type</button>
                        </div>
                    </form>
                </div>
...
@push('scripts')
    <script>
        ...
        function editRecord(record) {
            ...
            bootstrap.Modal.getOrCreateInstance(document.getElementById('surgeryFormModal')).show();
        }

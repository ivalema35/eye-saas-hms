...
@section('title', 'Lens Options Master')
@section('page-header', 'Lens Options Master')
...
<div class="ot-master-page">
    <div class="ot-master-toolbar">
        ...
        <button type="button"
                ...
                data-bs-target="#lensFormModal"
                onclick="resetForm()">
            <i class="bi bi-plus-lg me-1"></i> Add Lens Option
        </button>
    </div>
        ...
        <div class="card-body p-0">
            <div class="table-responsive ot-master-table-wrap">
                <table class="table table-hover align-middle mb-0 ot-master-table">
                        ...
                        <tr>
                            ...
                            <td class="text-end pe-4">
                                <div class="btn-group shadow-sm rounded-3" role="group">
                                    ...
                                    <form method="POST" class="d-inline" action="{{ route('hospital.masters.ot.lens-options.destroy', ['slug' => $slug, 'id' => $record->id]) }}" onsubmit="return confirm('Delete this lens option?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="text-center py-4 text-muted">No lens options found.</td>
                        </tr>
                    @endforelse
                    </tbody>
                </table>
    ...
    <div class="modal fade ot-master-modal" id="lensFormModal" tabindex="-1" aria-hidden="true">
            ...
            <div class="modal-content border-0">
                <div class="modal-header border-0 pb-0">
                    <div class="d-flex align-items-center">
                        ...
                        <h5 class="modal-title fw-bold mb-0" id="formTitle">Add Lens Option</h5>
                    </div>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>

                <form id="lensForm" method="POST" action="{{ route('hospital.masters.ot.lens-options.store', ['slug' => $slug]) }}">
                    @csrf
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <input type="text" name="name" id="name" class="form-control form-control-lg ot-master-input" value="{{ old('name') }}" required>
                        </div>
                    ...
                    <div class="modal-footer border-0 gap-2">
                        ...
                        <button class="btn btn-primary fw-semibold rounded-3" type="submit" id="submitBtn">Save Lens Option</button>
                    </div>
                </form>
            </div>
        </div>
...
@push('scripts')
<script>
    const storeUrl = "{{ route('hospital.masters.ot.lens-options.store', ['slug' => $slug]) }}";
    const updateUrl = "{{ route('hospital.masters.ot.lens-options.update', ['slug' => $slug, 'id' => '__ID__']) }}";
    ...
    function editRecord(record) {
        ...
        document.getElementById('submitBtn').innerText = 'Update Lens Option';
        document.getElementById('formTitle').innerText = 'Edit Lens Option';
        document.getElementById('formIcon').className = 'bi bi-pencil-square';
        bootstrap.Modal.getOrCreateInstance(document.getElementById('lensFormModal')).show();
    }
    ...
    function resetForm() {
        ...
        document.getElementById('submitBtn').innerText = 'Save Lens Option';
        document.getElementById('formTitle').innerText = 'Add Lens Option';

...
@section('title', 'Lens Power Master')
@section('page-header', 'Lens Power Master')
...
@section('content')
    <div class="ot-master-page">
                        ...
                        <tbody>
                            @forelse($records as $index => $record)
                                <tr>
                                    ...
                                    <td>+{{ number_format((float) $record->power, 2) }} D</td>
                                    <td>
                                        @if($record->is_favourite)
                                            <span class="badge bg-warning-subtle text-warning"><i class="bi bi-star-fill"></i> Favourite</span>
                                        @else
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="text-center py-4 text-muted">No lens powers found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        <div class="modal fade ot-master-modal" id="lensPowerFormModal" tabindex="-1" aria-hidden="true">
                ...
                <div class="modal-content border-0">
                    <div class="modal-header border-0 pb-0">
                        <div class="d-flex align-items-center">
                            ...
                            <h5 class="modal-title fw-bold mb-0" id="formTitle">Add Lens Power</h5>
                        </div>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>

                    <form id="lensPowerForm" method="POST"
                        action="{{ route('hospital.masters.ot.lens-powers.store', ['slug' => $slug]) }}">
                        @csrf
                        <input type="hidden" name="_method" id="formMethod" value="POST">

                        <div class="modal-body">
                            <div class="mb-3">
                                <label class="form-label text-muted small fw-bold text-uppercase">Power (D)</label>
                                <input type="number" step="0.01" name="power" id="power"
                                    class="form-control form-control-lg ot-master-input" value="{{ old('power') }}" required>
                            </div>

                            <div class="form-check ot-master-check mb-2">
                                <input type="checkbox" class="form-check-input" id="is_favourite" name="is_favourite" value="1">
                                <label class="form-check-label" for="is_favourite">Mark as Favourite</label>
                            </div>
                        ...
                        <div class="modal-footer border-0 gap-2">
                            ...
                            <button class="btn btn-primary fw-semibold rounded-3" type="submit" id="submitBtn">Save Lens
                                Power</button>
                        </div>
                    </form>
                </div>
...
@push('scripts')
    <script>
        const storeUrl = "{{ route('hospital.masters.ot.lens-powers.store', ['slug' => $slug]) }}";
        const updateUrl = "{{ route('hospital.masters.ot.lens-powers.update', ['slug' => $slug, 'id' => '__ID__']) }}";

        function editRecord(record) {
            const form = document.getElementById('lensPowerForm');
            form.action = updateUrl.replace('__ID__', record.id);
            document.getElementById('formMethod').value = 'PUT';
            document.getElementById('power').value = record.power ?? '';
            document.getElementById('is_favourite').checked = Boolean(record.is_favourite);
            document.getElementById('is_active').checked = Boolean(record.is_active);
            document.getElementById('submitBtn').innerText = 'Update Lens Power';
            document.getElementById('formTitle').innerText = 'Edit Lens Power';
            document.getElementById('formIcon').className = 'bi bi-pencil-square';
            bootstrap.Modal.getOrCreateInstance(document.getElementById('lensPowerFormModal')).show();
        }
        ...
        function resetForm() {
            ...
            document.getElementById('submitBtn').innerText = 'Save Lens Power';
            document.getElementById('formTitle').innerText = 'Add Lens Power';

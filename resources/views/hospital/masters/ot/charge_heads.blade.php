...
@section('content')
    <div class="ot-master-page">
        <div class="ot-master-toolbar">
            ...
            @haspermission('ot_charge_add')
            <button type="button" class="btn btn-primary ot-master-add-btn" data-bs-toggle="modal"
                data-bs-target="#chargeFormModal" onclick="resetForm()">
                <i class="bi bi-plus-lg me-1"></i> Add Charge Head
            </button>
            @endhaspermission
        </div>
                                ...
                                <tr>
                                    ...
                                    <td class="text-end pe-4">
                                        <div class="btn-group shadow-sm rounded-3" role="group">
                                            ...
                                            @endhaspermission
                                            @haspermission('ot_charge_delete')
                                            <form method="POST" class="d-inline"
                                                action="{{ route('hospital.masters.ot.charge-heads.destroy', ['slug' => $slug, 'id' => $record->id]) }}"
                                                onsubmit="return confirm('Delete this charge head?');">
                                                @csrf
                                                @method('DELETE')
        ...
        <div class="modal fade ot-master-modal" id="chargeFormModal" tabindex="-1" aria-hidden="true">
                ...
                <div class="modal-content border-0">
                    <div class="modal-header border-0 pb-0">
                        <div class="d-flex align-items-center">
                            ...
                            <h5 class="modal-title fw-bold mb-0" id="formTitle">Add OT Charge Head</h5>
                        </div>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>

                    <form id="chargeForm" method="POST"
                        action="{{ route('hospital.masters.ot.charge-heads.store', ['slug' => $slug]) }}">
                        @csrf
                        <input type="hidden" name="_method" id="formMethod" value="POST">

                        <div class="modal-body">
                            <div class="mb-3">
                                <label class="form-label text-muted small fw-bold text-uppercase">Charge Name</label>
                                <input type="text" name="charge_name" id="charge_name"
                                    class="form-control form-control-lg ot-master-input" value="{{ old('charge_name') }}"
                                    required>
                            </div>

                            <div class="mb-3">
                                <label class="form-label text-muted small fw-bold text-uppercase">Percentage</label>
                                <input type="number" step="0.01" min="0" max="100" name="percentage" id="percentage"
                                    class="form-control form-control-lg ot-master-input" value="{{ old('percentage') }}"
                                    required>
                            </div>
                        ...
                        <div class="modal-footer border-0 gap-2">
                            ...
                            <button class="btn btn-primary fw-semibold rounded-3" type="submit" id="submitBtn">Save Charge
                                Head</button>
                        </div>
                    </form>
                </div>
...
@push('scripts')
    <script>
        ...
        function editRecord(record) {
            ...
            bootstrap.Modal.getOrCreateInstance(document.getElementById('chargeFormModal')).show();
        }

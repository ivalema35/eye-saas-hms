                            ...
                            <tbody id="rxBody">
                                @forelse($initialMedicines as $i => $rx)
                                    <tr class="rx-row">
                                        ...
                                        <td style="padding:8px 14px;"><input type="number" name="medicines[{{ $i }}][quantity]" class="form-control form-control-sm" value="{{ data_get($rx, 'quantity', '') }}" placeholder="1" min="1"></td>
                                        <td style="padding:8px 14px;">
                ...
                <div class="modal-body p-3 d-flex flex-column gap-3">
                    ...
                    <div class="rounded-3 p-3" style="border:1px solid #dde3ea;background:white;">
                        ...
                        <textarea name="exam_data[advice]" id="advice_textarea" class="form-control" rows="8"
                                  ...
                                  maxlength="2000"
                                  style="resize:vertical;font-size:13px;border-color:#e2e8f0;border-radius:8px;line-height:1.7;">{{ old('exam_data.advice', $ed['advice'] ?? $secondaryExam?->advice ?? '') }}</textarea>
                    </div>
...
$medicinesForJs = $masters['medicines']->map(fn($m) => [
    ...
    window.updateLivePreview = function () {
        ...
        // ST table (navy theme, per-eye ADD, VN removed)
...
$__dxAdvices = $masters['advices']->map(fn($a) => ['id' => $a->id, 'advice' => $a->advice ?? '', 'diagnosis_ids' => $a->diagnosis_ids ?? []])->values();
    ...
    (function () {
        ...
        function appendAdviceChip(text) {
            ...
            if (chipsWrap && moreDrop) chipsWrap.insertBefore(chip, moreDrop);
            ...
            newLi.innerHTML = '<button type="button" class="advice-quick-btn" data-advice="' + text.replace(/"/g, '&quot;') + '" style="flex:1;font-size:13px;padding:6px 4px;text-align:left;border:none;background:transparent;color:#1e293b;cursor:pointer;border-radius:6px;line-height:1.4;">' + text + '</button>';

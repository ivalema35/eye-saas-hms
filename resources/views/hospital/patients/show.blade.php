...
<div style="max-width:780px;margin:0 auto">
    ...
    <div class="hms-card border-0 shadow-sm" style="border-radius:16px;overflow:hidden;margin-bottom:1.25rem">
        <div style="background:linear-gradient(135deg,#1B4F72,#2980B9);padding:1.5rem 1.75rem;display:flex;align-items:center;gap:1rem">
            <div style="width:52px;height:52px;background:rgba(255,255,255,.2);border-radius:14px;display:flex;align-items:center;justify-content:center;font-size:1.5rem;color:#fff;flex-shrink:0">
            ...
            <div>
                @if($patient->case_id)
                    <span style="background:rgba(39,174,96,.25);color:#D5F5E3;border:1px solid rgba(39,174,96,.4);border-radius:20px;padding:.3rem .9rem;font-size:.75rem;font-weight:700">
                        <i class="bi bi-check-circle-fill"></i> Checked In
                    </span>
                @else
                    <span style="background:rgba(255,193,7,.2);color:#FFEAA7;border:1px solid rgba(255,193,7,.35);border-radius:20px;padding:.3rem .9rem;font-size:.75rem;font-weight:700">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.25rem">
        ...
        <div class="hms-card border-0 shadow-sm" style="border-radius:14px;padding:1.25rem 1.5rem">
            <div style="font-size:.7rem;font-weight:800;text-transform:uppercase;letter-spacing:.07em;color:#64748B;margin-bottom:1rem">
            ...
            <div style="display:flex;flex-direction:column;gap:.65rem">
                ...
                <div>
                    ...
                    <div style="font-weight:600;color:#1A202C">{{ $patient->location?->city ?: '—' }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="hms-card border-0 shadow-sm" style="border-radius:14px;padding:1.25rem 1.5rem">
            ...
            <div style="display:flex;flex-direction:column;gap:.65rem">
                <div>
                    ...
                    <div style="font-weight:700;color:#1A202C">{{ $patient->created_at?->format('d M Y, h:i A') ?? '—' }}</div>
                </div>
                <div>
    ...
    @if(!$patient->case_id)
    <div class="hms-card border-0 shadow-sm" style="border-radius:14px;padding:1.25rem 1.5rem;margin-top:1.25rem;display:flex;align-items:center;justify-content:space-between;gap:1rem;background:linear-gradient(135deg,#FFF9E6,#FFFDE7)">
        <div>
            <div style="font-weight:700;color:#856404"><i class="bi bi-exclamation-circle-fill me-1"></i> Patient not yet checked in</div>
            <div style="font-size:.8rem;color:#A07800;margin-top:.2rem">Assign case type and fee to complete check-in</div>
        </div>

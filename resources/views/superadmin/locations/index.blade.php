...
@section('page-header', 'Location Master')
...
@section('page-actions')
    ...
    @if($tab === 'countries')
        <button type="button" class="hms-btn hms-btn-primary hms-btn-sm ms-2" data-bs-toggle="modal" data-bs-target="#addCountryModal">
            <i class="bi bi-plus-lg"></i> Add Country
        </button>
    @elseif($tab === 'states')
        <button type="button" class="hms-btn hms-btn-primary hms-btn-sm ms-2" data-bs-toggle="modal" data-bs-target="#addStateModal">
            <i class="bi bi-plus-lg"></i> Add State
        </button>
    @elseif($tab === 'districts')
        <button type="button" class="hms-btn hms-btn-primary hms-btn-sm ms-2" data-bs-toggle="modal" data-bs-target="#addDistrictModal">
            <i class="bi bi-plus-lg"></i> Add District
        </button>
    @elseif($tab === 'cities')
        <button type="button" class="hms-btn hms-btn-primary hms-btn-sm ms-2" data-bs-toggle="modal" data-bs-target="#addCityModal">
...
<style>
    ...
    .loc-tab-btn:hover { color:#1B4F72; }
    .loc-tab-btn.active { color:#1B4F72; border-bottom-color:#1B4F72; }
    ...
    .loc-empty { padding:3rem; text-align:center; color:#94A3B8; }
    .loc-empty i { font-size:2.5rem; display:block; margin-bottom:.75rem; }
    .loc-empty p { margin:0; font-weight:600; }
    .loc-hint { margin:.25rem 0 0; font-size:.85rem; }
...
<div class="loc-tab-nav">
    <a href="{{ route('superadmin.locations.index', ['tab' => 'countries']) }}"
       class="loc-tab-btn {{ $tab === 'countries' ? 'active' : '' }}">
        ...
        <span class="badge-count">{{ $countries->count() }}</span>
    </a>
...
<div class="hms-card" style="padding:0">
    <div class="hms-card-header">
        ...
        <span class="hms-badge hms-badge-info">{{ $countries->count() }} total</span>
    </div>
...
<div class="filter-bar">
    <form method="GET" action="{{ route('superadmin.locations.index') }}" style="display:flex;gap:.75rem;flex-wrap:wrap;align-items:flex-end">
        ...
        <div style="display:flex;gap:.5rem">
            ...
            <a href="{{ route('superadmin.locations.index', ['tab' => 'states']) }}" class="hms-btn hms-btn-outline hms-btn-sm">Clear</a>
        </div>
    </form>
</div>
...
<div class="hms-card" style="padding:0">
    <div class="hms-card-header">
        <h3 class="hms-card-title"><i class="bi bi-map" style="color:#1B4F72"></i> State List</h3>
    ...
    @if(! $filterCountryId)
        <div class="loc-empty"><i class="bi bi-arrow-up-circle"></i><p>Select a country above to view states</p></div>
    @elseif($states->isEmpty())
        <div class="loc-empty"><i class="bi bi-map"></i><p>No states found</p><p class="loc-hint">Click "Add State" to add one.</p></div>
    @else
        <div class="hms-table-wrap" style="border:none">
            <table class="hms-table">
                <thead><tr><th>#</th><th>State Name</th><th>Country</th><th>Districts</th><th>Status</th><th class="text-end">Actions</th></tr></thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <button class="hms-badge toggle-btn {{ $s->is_active ? 'hms-badge-success' : 'hms-badge-secondary' }}"
                        ...
                        <td class="text-end">
                            <div style="display:flex;gap:.4rem;justify-content:flex-end">
                                <button class="hms-btn hms-btn-outline hms-btn-xs edit-state-btn"
                                    data-id="{{ $s->id }}" data-name="{{ $s->name }}" data-country-id="{{ $s->country_id }}">
                                    <i class="bi bi-pencil-fill"></i>
                                </button>
                                <form method="POST" action="{{ route('superadmin.locations.states.destroy', $s->id) }}" class="del-form">
                                    @csrf @method('DELETE')
...
@if($tab === 'districts')
<div class="filter-bar">
    <form method="GET" action="{{ route('superadmin.locations.index') }}" style="display:flex;gap:.75rem;flex-wrap:wrap;align-items:flex-end" id="districtFilterForm">
        ...
        <div style="display:flex;gap:.5rem">
            <button type="submit" class="hms-btn hms-btn-primary hms-btn-sm"><i class="bi bi-search"></i> Filter</button>
            <a href="{{ route('superadmin.locations.index', ['tab' => 'districts']) }}" class="hms-btn hms-btn-outline hms-btn-sm">Clear</a>
        </div>
    </form>
</div>
...
<div class="hms-card" style="padding:0">
    <div class="hms-card-header">
        <h3 class="hms-card-title"><i class="bi bi-geo" style="color:#1B4F72"></i> District List</h3>
    ...
    @if(! $filterStateId)
        <div class="loc-empty"><i class="bi bi-arrow-up-circle"></i><p>Select country &amp; state above to view districts</p></div>
    @elseif($districts->isEmpty())
        <div class="loc-empty"><i class="bi bi-geo"></i><p>No districts found</p><p class="loc-hint">Click "Add District" to add one.</p></div>
    @else
        <div class="hms-table-wrap" style="border:none">
            <table class="hms-table">
                <thead><tr><th>#</th><th>District Name</th><th>State</th><th>Cities</th><th>Status</th><th class="text-end">Actions</th></tr></thead>
                <tbody>
                        ...
                        <td class="text-end">
                            <div style="display:flex;gap:.4rem;justify-content:flex-end">
                                <button class="hms-btn hms-btn-outline hms-btn-xs edit-district-btn"
                                    data-id="{{ $d->id }}" data-name="{{ $d->name }}" data-state-id="{{ $d->state_id }}">
                                    <i class="bi bi-pencil-fill"></i>
                                </button>
                                <form method="POST" action="{{ route('superadmin.locations.districts.destroy', $d->id) }}" class="del-form">
                                    @csrf @method('DELETE')
                                    <button class="hms-btn hms-btn-xs" style="background:#FEE2E2;color:#B91C1C;border:none"><i class="bi bi-trash-fill"></i></button>
                                </form>
                            </div>
...
@if($tab === 'cities')
<div class="filter-bar">
    <form method="GET" action="{{ route('superadmin.locations.index') }}" style="display:flex;gap:.75rem;flex-wrap:wrap;align-items:flex-end" id="cityFilterForm">
        ...
        <div style="display:flex;gap:.5rem">
            <button type="submit" class="hms-btn hms-btn-primary hms-btn-sm"><i class="bi bi-search"></i> Filter</button>
            <a href="{{ route('superadmin.locations.index', ['tab' => 'cities']) }}" class="hms-btn hms-btn-outline hms-btn-sm">Clear</a>
        </div>
    </form>
</div>
...
<div class="hms-card" style="padding:0">
    <div class="hms-card-header">
        <h3 class="hms-card-title"><i class="bi bi-building" style="color:#1B4F72"></i> City / Village List</h3>
    ...
    @if(! $filterStateId)
        <div class="loc-empty"><i class="bi bi-arrow-up-circle"></i><p>Select country &amp; state above to view cities</p></div>
    @elseif($cities->isEmpty())
        <div class="loc-empty"><i class="bi bi-building"></i><p>No cities found</p><p class="loc-hint">Click "Add City" to add one.</p></div>
    @else
        <div class="hms-table-wrap" style="border:none">
            <table class="hms-table">
                <thead><tr><th>#</th><th>City / Village</th><th>District</th><th>State</th><th>Status</th><th class="text-end">Actions</th></tr></thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <button class="hms-badge toggle-btn {{ $ci->is_active ? 'hms-badge-success' : 'hms-badge-secondary' }}"
                        ...
                        <td class="text-end">
                            <div style="display:flex;gap:.4rem;justify-content:flex-end">
                                ...
                                <form method="POST" action="{{ route('superadmin.locations.cities.destroy', $ci->id) }}" class="del-form">
                                    @csrf @method('DELETE')
                                    <button class="hms-btn hms-btn-xs" style="background:#FEE2E2;color:#B91C1C;border:none"><i class="bi bi-trash-fill"></i></button>
                                </form>
                            </div>
...
<div class="modal fade" id="addStateModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius:12px;border:none;box-shadow:0 20px 60px rgba(0,0,0,.15)">
            <div class="modal-header" style="background:#1B4F72;border-radius:12px 12px 0 0;padding:.875rem 1.25rem">
                <h5 class="modal-title" style="color:#fff;font-weight:700;font-size:.95rem"><i class="bi bi-map me-2"></i>Add State</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <form method="POST" action="{{ route('superadmin.locations.states.store') }}">
                ...
                <div class="modal-footer" style="border-top:1px solid #F1F5F9;padding:.75rem 1.25rem">
                    <button type="button" class="hms-btn hms-btn-outline hms-btn-sm" data-bs-dismiss="modal">Cancel</button>
                    <button type="submit" class="hms-btn hms-btn-primary hms-btn-sm"><i class="bi bi-plus-lg"></i> Add State</button>
                </div>
            </form>
        </div>
    </div>
...
<div class="modal fade" id="editStateModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius:12px;border:none;box-shadow:0 20px 60px rgba(0,0,0,.15)">
            <div class="modal-header" style="background:#1B4F72;border-radius:12px 12px 0 0;padding:.875rem 1.25rem">
                <h5 class="modal-title" style="color:#fff;font-weight:700;font-size:.95rem"><i class="bi bi-pencil-fill me-2"></i>Edit State</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
...
<div class="modal fade" id="addDistrictModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius:12px;border:none;box-shadow:0 20px 60px rgba(0,0,0,.15)">
            <div class="modal-header" style="background:#1B4F72;border-radius:12px 12px 0 0;padding:.875rem 1.25rem">
                <h5 class="modal-title" style="color:#fff;font-weight:700;font-size:.95rem"><i class="bi bi-geo me-2"></i>Add District</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <form method="POST" action="{{ route('superadmin.locations.districts.store') }}">
                ...
                <div class="modal-footer" style="border-top:1px solid #F1F5F9;padding:.75rem 1.25rem">
                    <button type="button" class="hms-btn hms-btn-outline hms-btn-sm" data-bs-dismiss="modal">Cancel</button>
                    <button type="submit" class="hms-btn hms-btn-primary hms-btn-sm"><i class="bi bi-plus-lg"></i> Add District</button>
                </div>
            </form>
        </div>
    </div>
...
<div class="modal fade" id="editDistrictModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius:12px;border:none;box-shadow:0 20px 60px rgba(0,0,0,.15)">
            <div class="modal-header" style="background:#1B4F72;border-radius:12px 12px 0 0;padding:.875rem 1.25rem">
                <h5 class="modal-title" style="color:#fff;font-weight:700;font-size:.95rem"><i class="bi bi-pencil-fill me-2"></i>Edit District</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
...
<div class="modal fade" id="addCityModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius:12px;border:none;box-shadow:0 20px 60px rgba(0,0,0,.15)">
            <div class="modal-header" style="background:#1B4F72;border-radius:12px 12px 0 0;padding:.875rem 1.25rem">
                <h5 class="modal-title" style="color:#fff;font-weight:700;font-size:.95rem"><i class="bi bi-building me-2"></i>Add City / Village</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <form method="POST" action="{{ route('superadmin.locations.cities.store') }}">
                ...
                <div class="modal-body" style="padding:1.25rem">
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            <label class="hms-label">District <span style="color:#94A3B8;font-weight:400">(optional)</span></label>
                ...
                <div class="modal-footer" style="border-top:1px solid #F1F5F9;padding:.75rem 1.25rem">
                    <button type="button" class="hms-btn hms-btn-outline hms-btn-sm" data-bs-dismiss="modal">Cancel</button>
                    <button type="submit" class="hms-btn hms-btn-primary hms-btn-sm"><i class="bi bi-plus-lg"></i> Add City</button>
                </div>
            </form>
        </div>
    </div>
...
<div class="modal fade" id="editCityModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius:12px;border:none;box-shadow:0 20px 60px rgba(0,0,0,.15)">
            <div class="modal-header" style="background:#1B4F72;border-radius:12px 12px 0 0;padding:.875rem 1.25rem">
                <h5 class="modal-title" style="color:#fff;font-weight:700;font-size:.95rem"><i class="bi bi-pencil-fill me-2"></i>Edit City</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
...
<div class="modal fade" id="importModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius:12px;border:none;box-shadow:0 20px 60px rgba(0,0,0,.15)">
            <div class="modal-header" style="background:#1B4F72;border-radius:12px 12px 0 0;padding:.875rem 1.25rem">
                <h5 class="modal-title" style="color:#fff;font-weight:700;font-size:.95rem"><i class="bi bi-file-earmark-arrow-up me-2"></i>Import via Excel</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
                ...
                <div class="modal-body" style="padding:1.25rem">
                    <div style="background:#EFF6FF;border:1px solid #BFDBFE;border-radius:8px;padding:1rem;margin-bottom:1.25rem">
                        ...
                        <div style="margin-top:.5rem;display:flex;gap:.4rem;flex-wrap:wrap">
                            <code style="background:#DBEAFE;color:#1D4ED8;padding:.15rem .4rem;border-radius:4px;font-size:.78rem">country</code>
                            <code style="background:#DBEAFE;color:#1D4ED8;padding:.15rem .4rem;border-radius:4px;font-size:.78rem">state</code>
                            <code style="background:#F3F4F6;color:#6B7280;padding:.15rem .4rem;border-radius:4px;font-size:.78rem">district</code>
                            <code style="background:#DBEAFE;color:#1D4ED8;padding:.15rem .4rem;border-radius:4px;font-size:.78rem">city</code>
                        </div>

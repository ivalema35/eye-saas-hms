...
list all sit inside one bordered card, matching the Medicine Master / OT
panel design. --}}
...
@push('styles')
    <style>
        ...
        .tzm-header-block { padding:1.25rem 1.5rem 1rem; }
        .tzm-header-title {
            font-weight:800; font-size:1.3rem; color:#1B4F72;
            letter-spacing:-.015em; display:flex; align-items:center; gap:.55rem;
        }
        .tzm-header-title i { color:#1B4F72; font-size:1.2rem; }
        .tzm-breadcrumb {
            margin-top:.4rem; display:flex; align-items:center; gap:.4rem;
            font-size:.85rem; color:#8891A0;
        }
        .tzm-breadcrumb a { color:#8891A0; text-decoration:none; }
        .tzm-breadcrumb a:hover { color:#1B4F72; }
        .tzm-breadcrumb-sep { color:#C3C9D3; }
        .tzm-breadcrumb-current { color:#4A5568; font-weight:600; }
        .tzm-panel-body { padding:0 1.5rem 1.5rem; }
        .tzm-panel-body .hms-card { margin-bottom:0; }
    </style>
@endpush

@section('content')
<div class="tzm-premium-card">

    <div class="tzm-header-block">
        <div class="tzm-header-title"><i class="bi bi-clock-fill"></i> Timezone Master</div>
        <nav class="tzm-breadcrumb" aria-label="breadcrumb">
            <a href="{{ route('superadmin.dashboard') }}">Home</a>
            <span class="tzm-breadcrumb-sep">/</span>
            <span>Masters</span>
            <span class="tzm-breadcrumb-sep">/</span>
            <span class="tzm-breadcrumb-current">Timezone Master</span>
        </nav>
    </div>

    <div class="tzm-panel-body">
    <div class="hms-card" style="padding:0">
        <div class="hms-card-header">
            <h3 class="hms-card-title">
                <i class="bi bi-clock-fill" style="color:#ffffff"></i> All Timezones
            </h3>
            ...
            <div style="display:flex;gap:.75rem;align-items:center">
                <div style="position:relative;flex:1;max-width:400px">
                    <i class="bi bi-search"
                        style="position:absolute;left:.75rem;top:50%;transform:translateY(-50%);color:#94A3B8;pointer-events:none"></i>
                    <input type="text" id="tzSearchInput" placeholder="Search... (e.g. Kolkata, New_York, London)"
            ...
            @foreach($groupedTimezones as $region => $zones)
                <div class="tz-region-block" style="margin-bottom:1.75rem">
                    ...
                    <div style="display:flex;flex-wrap:wrap;gap:.4rem">
                        @foreach($zones as $tz)
                            <span class="tz-item" data-tz="{{ strtolower($tz) }}"
                                title="{{ \Carbon\Carbon::now($tz)->format('d M Y, h:i A') }}" style="display:inline-flex;align-items:center;gap:.35rem;

    ...
    <div class="hms-card" style="padding:1.25rem;margin-bottom:1.25rem">
        ...
        <form method="GET" style="display:flex;gap:.75rem;flex-wrap:wrap;align-items:flex-end">
            <div class="hms-form-group" style="margin-bottom:0;flex:1;min-width:180px">
                ...
                <input type="text" name="action" class="hms-input" value="{{ request('action') }}"
                    placeholder="e.g. hospital.suspended">
            </div>
            <div class="hms-form-group" style="margin-bottom:0;min-width:180px">
                ...
                <select name="tenant_id" class="hms-select">
                    ...
                    @foreach($tenants as $t)
                        <option value="{{ $t->id }}" {{ request('tenant_id') == $t->id ? 'selected' : '' }}>{{ $t->name }}
                        </option>
                    @endforeach
                </select>
            </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="text-center py-5">
                                <x-empty-state icon="bi bi-clock-history" title="No Audit Logs Found"

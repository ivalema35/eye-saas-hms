            ...
            <div style="padding:1.5rem">
                <form method="POST" action="{{ route('superadmin.notifications.send') }}" id="notifyForm">
                    ...
                    <div class="hms-form-group">
                        <label class="hms-label">Subject <span style="color:#C0392B">*</span></label>
                        <input type="text" name="subject" class="hms-input @error('subject') is-invalid @enderror"
                            placeholder="e.g. Important: Platform Update" value="{{ old('subject') }}" required>
                        @error('subject')
                    ...
                    <div class="hms-form-group">
                        <label class="hms-label">Message <span style="color:#C0392B">*</span></label>
                        <textarea name="message" class="hms-input @error('message') is-invalid @enderror" rows="6"
                    ...
                    <div class="hms-form-group">
                        <label class="hms-label">Recipients <span style="color:#C0392B">*</span></label>
                        <select name="recipient" class="hms-select" id="recipientSelect" required>
                            <option value="all" {{ old('recipient') === 'all' ? 'selected' : '' }}>All Hospitals
                                ({{ $tenants->count() }} hospitals)</option>
                            <option value="specific" {{ old('recipient') === 'specific' ? 'selected' : '' }}>Specific
                                Hospitals</option>
                        </select>
                    </div>

                    <div class="hms-form-group" id="specificTenantsGroup" style="display:none">
                        <label class="hms-label">Select Hospitals <span style="color:#C0392B">*</span></label>
                        <select name="tenant_ids[]" class="hms-select @error('tenant_ids') is-invalid @enderror"
                            id="tenantSelect" multiple style="height:auto">
                            @foreach($tenants as $tenant)
                                <option value="{{ $tenant->id }}" {{ is_array(old('tenant_ids')) && in_array($tenant->id, old('tenant_ids')) ? 'selected' : '' }}>
                    ...
                    <button type="submit" class="hms-btn hms-btn-primary" style="width:100%;color: #1b4f72;" id="sendBtn"
                        ...
                        <tbody>
                            @forelse($recentNotifications as $notif)
                                <tr>
                                    <td>
                                        @if($notif->tenant)
                                            <a href="{{ route('superadmin.hospitals.show', $notif->tenant) }}"
                                                style="font-weight:600;font-size:.85rem;color:#1A202C">
                                                {{ $notif->tenant->name }}
                                            </a>
                                            <div style="font-size:.72rem;color:#64748B">{{ $notif->tenant->slug }}</div>
                                        @else
                                            <span style="color:#94A3B8;font-size:.82rem">Deleted Hospital</span>
                                        @endif
                                    </td>
                                    <td style="font-size:.85rem;max-width:200px;color:#334155">
                                        {{ \Illuminate\Support\Str::limit($notif->subject, 50) }}
                                    </td>
                                    <td style="font-size:.8rem;color:#64748B">{{ $notif->recipient_email }}</td>
                                    <td>
                                        @if($notif->status === 'sent')
                                            <span class="hms-badge hms-badge-active">Sent</span>
                                        @elseif($notif->status === 'failed')
                                            <span class="hms-badge hms-badge-suspended"
                                                title="{{ $notif->error_message }}">Failed</span>
                                        @else
                                            <span class="hms-badge hms-badge-inactive">Pending</span>
                                        @endif
                                    </td>
                                    <td style="font-size:.8rem;white-space:nowrap;color:#475569">
